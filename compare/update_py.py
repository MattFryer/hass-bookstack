...
Compares the version of the connected BookStack instance (returned by the
/api/system endpoint and stored on the coordinator) against the latest
release published on the BookStack GitHub repository.  The entity surfaces
inside the HA Updates panel and supports:

  - Showing the installed and latest version strings.
  - Providing a "Read release announcement" link pointing directly to the
    GitHub release page for the latest version.

Performing the actual upgrade from within Home Assistant is not possible
(BookStack must be updated on the server), so the INSTALL feature flag and
install() method are intentionally omitted.

Skipping a release is handled natively by Home Assistant and does not
require any custom logic in this integration.
"""
...
class BookStackUpdateEntity(CoordinatorEntity[BookStackCoordinator], UpdateEntity):
    ...
    Supports RELEASE_NOTES to populate the "Read release announcement" link
    in HA's update dialog.

    INSTALL is intentionally absent — BookStack cannot be upgraded from
    within Home Assistant; it must be updated on the server.

    Skipping a release is handled natively by Home Assistant and requires no
    custom logic here.
    """
    ...
    def latest_version(self) -> str | None:
        ...
        Returns None while the GitHub check is still pending, which causes
        HA to render the entity state as 'unknown'.
        """
    ...
    def release_url(self) -> str | None:
        ...
        HA renders this as the "Read release announcement" link inside the
        update dialog.
        """
    ...
    async def async_release_notes(self) -> str | None:
        ...
        if url:
            return (
                ...
                "See the full changelog and upgrade instructions on the "
